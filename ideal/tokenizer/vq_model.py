...
class VQModel(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.config = config
        self.fusion_mode = getattr(config.transformer_config.encoder_config.params, "fusion_mode", None)
        self.latent_channels = config.z_channels

        if self.fusion_mode == "channel_concat":
            self.latent_channels = config.z_channels * 2
        ...
        self.pre_slots_quant = nn.Linear(self.latent_channels, config.codebook_slots_embed_dim)
        self.post_slots_quant = nn.Linear(config.codebook_slots_embed_dim, self.latent_channels)
    ...
    def encode(self, x):
        ...
        slots, spatial, semantic = self.encode_transformer(x,)
        ...
        return (quant2, semantic, spatial), emb_loss, q_indices
    ...
    def decode_from_indices(self, q_indices, z_shape=None, channel_first=False):
        ...
        dec, semantic_recon, spatial_recon = self.decode(slots)

        return dec, semantic_recon, spatial_recon

    def decode_codes_to_img(self, codes, tgt_size):

        bs, num = codes.shape
        qz_shape = (bs, num, self.config.codebook_slots_embed_dim,)
        results, _, _ = self.decode_code(codes, qz_shape, False)
    ...
    def decode(self, slots):
        ...
        z, semantic_recon, spatial_recon = self.decode_transformer(queries)
        dec = self.decoder(z)
        return dec, semantic_recon, spatial_recon
    ...
    def decode_code(self, code_b, shape=None, channel_first=True):
        ...
        dec, semantic_recon, spatial_recon = self.decode(quant_b)
        return dec, semantic_recon, spatial_recon

    def forward(self, input):

        (quant, semantic, spatial), diff, q_indices = self.encode(input)

        if self.training:
            dec, semantic_recon, spatial_recon = self.decode(quant)
        else:
            dec, semantic_recon, spatial_recon = self.decode_from_indices(q_indices, quant.shape)
        return (dec, semantic ,spatial ,semantic_recon, spatial_recon), diff, q_indices
