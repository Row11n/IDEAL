...
from ideal.engine.util import disabled_train
import kornia
import os
...
class TimmEmbedder(nn.Module):
    ...
    def __init__(
        ...
        if self.embed_dim is None:
            raise ValueError("TimmEmbedder requires backbone with 'embed_dim' or 'num_features'.")
        # shallow tokens receive an independent LN, deep tokens keep the backbone norm
        self.spatial_norm = nn.LayerNorm(self.embed_dim, eps=1e-6)
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = kornia.geometry.resize(
            x,
            (self.img_size, self.img_size), # always change resolution here
    ...
    def forward(self, x: torch.Tensor):
        ...
        if len(feats) == 1:
            semantic = self.model.norm(feats[0])
            return [None, semantic]

        spatial = self.spatial_norm(feats[0])
        semantic = self.model.norm(feats[1])
