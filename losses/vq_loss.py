...
class VQLoss(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx, global_step, last_layer=None, 
        ...
        if optimizer_idx == 0:
            ...
            semantic_cos_loss = codebook_loss[4] if len(codebook_loss) > 4 else None
            semantic_l2_loss = codebook_loss[5] if len(codebook_loss) > 5 else None
            shallow_cos_loss = codebook_loss[6] if len(codebook_loss) > 6 else None
            shallow_l2_loss = codebook_loss[7] if len(codebook_loss) > 7 else None
            extra_losses = codebook_loss[8:] if len(codebook_loss) > 8 else ()

            if semantic_cos_loss is not None:
                loss += semantic_cos_loss
            if semantic_l2_loss is not None:
                loss += semantic_l2_loss
            if shallow_cos_loss is not None:
                loss += shallow_cos_loss
            if shallow_l2_loss is not None:
                loss += shallow_l2_loss
            for item in extra_losses:
                loss += item
            ...
            if global_step % log_every == 0:
                ...
                sem_cos_val = semantic_cos_loss if semantic_cos_loss is not None else 0.0
                sem_l2_val = semantic_l2_loss if semantic_l2_loss is not None else 0.0
                shallow_cos_val = shallow_cos_loss if shallow_cos_loss is not None else 0.0
                shallow_l2_val = shallow_l2_loss if shallow_l2_loss is not None else 0.0

                logger_string = f"(Generator) rec_loss: {rec_loss:.4f}, perceptual_loss: {p_loss:.4f}, " \
                                f"codebook_loss: {codebook_loss[0]:.4f}, commit_loss: {codebook_loss[1]:.4f}, entropy_loss: {codebook_loss[2]:.4f}"
                logger_string += f", semantic_cos: {sem_cos_val:.4f}, semantic_l2: {sem_l2_val:.4f}, shallow_cos: {shallow_cos_val:.4f}, shallow_l2: {shallow_l2_val:.4f}"
                logger_string += f", codebook_usage: {codebook_loss[3]:.4f}, generator_adv_loss: {generator_adv_loss:.4f}, " \
                                 f"disc_adaptive_weight: {disc_adaptive_weight:.4f}, disc_weight: {disc_weight:.4f}"
                if extra_losses:
                    for idx, item in enumerate(extra_losses, start=8):
                        logger_string += f", extra_loss_{idx}: {item:.4f}"
                ...
                if tdist.get_rank() == 0 and self.wandb_logger is not None:
                    log_dict = {
                        ...
                        "semantic_cos_loss": sem_cos_val,
                        "semantic_l2_loss": sem_l2_val,
                        "shallow_cos_loss": shallow_cos_val,
                        "shallow_l2_loss": shallow_l2_val,
                    }
                    if extra_losses:
                        for idx, item in enumerate(extra_losses, start=8):
                            log_dict[f"extra_loss_{idx}"] = item
        ...
        if optimizer_idx == 1:
            ...
            if global_step % log_every == 0:
                ...
                if tdist.get_rank() == 0 and self.wandb_logger is not None:
                    log_dict = {
                        "disc_adv_loss": d_adversarial_loss.detach(),
                        "disc_logits_real": logits_real,
                        "disc_logits_fake": logits_fake,
                    }
                    if self.disc_cr_loss_weight:
