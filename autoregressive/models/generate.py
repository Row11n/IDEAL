    ...
    **sampling_kwargs):
    ...
    for i in range(num_new_tokens):
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True): # Actually better for Inductor to codegen attention here
            ...
            next_token, next_prob = decode_one_token(
                model, cur_token[:, i:i+1], input_pos, cfg_scale, cfg_flag, **sampling_kwargs
            )
            input_pos += 1
    return input_pos
...
def generate(model, cond, max_new_tokens, emb_masks=None, cfg_scale=1.0, cfg_interval=-1, **sampling_kwargs):
    if model.model_type != 'c2i':
        raise ValueError("IDEAL release only supports class-conditional ImageNet AR generation.")
    if cfg_scale > 1.0:
