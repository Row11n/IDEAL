...
class SemanticEvaluator:
    ...
    def reset_metrics(self):
        ...
        self._l1_sum = 0.0
        self._l2_sum = 0.0
        self._spatial_examples = 0
        self._spatial_cos_sum = 0.0
        self._spatial_l1_sum = 0.0
        self._spatial_l2_sum = 0.0
        self._zs_top1 = 0
        self._zs_top5 = 0
        self._zs_total = 0
        self._set_of_codebook_indices = set()
    ...
    def update(
        ...
        teacher_spatial: Optional[torch.Tensor] = None,
        pred_spatial: Optional[torch.Tensor] = None,
    ):
        ...
        if teacher_spatial is not None and pred_spatial is not None:
            if teacher_spatial.shape != pred_spatial.shape:
                raise ValueError(
                    f"Teacher/pred spatial feature shapes mismatch: {teacher_spatial.shape} vs {pred_spatial.shape}"
                )
            t_spatial = teacher_spatial.to(self._device)
            p_spatial = pred_spatial.to(self._device)
            flat_t = t_spatial.view(t_spatial.size(0), -1)
            flat_p = p_spatial.view(p_spatial.size(0), -1)
            spatial_cos = F.cosine_similarity(flat_p, flat_t, dim=-1)
            spatial_l1 = F.l1_loss(flat_p, flat_t, reduction="none").mean(dim=-1)
            spatial_l2 = F.mse_loss(flat_p, flat_t, reduction="none").mean(dim=-1)
            self._spatial_examples += t_spatial.size(0)
            self._spatial_cos_sum += spatial_cos.sum().item()
            self._spatial_l1_sum += spatial_l1.sum().item()
            self._spatial_l2_sum += spatial_l2.sum().item()
    ...
    def result(self) -> Mapping[Text, float]:
        ...
        eval_score = {
            "Semantic_rCosine": self._cos_sum / self._num_examples,
            "Semantic_rL1": self._l1_sum / self._num_examples,
            "Semantic_rL2": self._l2_sum / self._num_examples,
        }
        if self._spatial_examples > 0:
            eval_score["Spatial_rCosine"] = self._spatial_cos_sum / self._spatial_examples
            eval_score["Spatial_rL1"] = self._spatial_l1_sum / self._spatial_examples
            eval_score["Spatial_rL2"] = self._spatial_l2_sum / self._spatial_examples
        if self._zs_total > 0:
